...
abstract class ClickItemTouchListener implements OnItemTouchListener {//单击项目触摸监听器
    ...
    private boolean isAttachedToWindow(RecyclerView hostView) {//是否附属于窗口
    ...
    @Override
    public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {//拦截触摸事件
    ...
    private class ItemClickGestureDetector extends GestureDetectorCompat {//项目点击 手势检测器
    ...
    private class ItemClickGestureListener extends SimpleOnGestureListener {//项目点击 手势监听器
        ...
        public void dispatchSingleTapUpIfNeeded(MotionEvent event) {//如果需要，调度单次轻敲
        ...
        @Override
        public void onShowPress(MotionEvent event) {//按压
            if (mTargetChild != null) {
        ...
        @Override
        public boolean onSingleTapUp(MotionEvent event) {//单次轻敲
        ...
        @Override
        public boolean onScroll(MotionEvent event, MotionEvent event2, float v, float v2) {//滚动
            if (mTargetChild != null) {
                mTargetChild.setPressed(false);
                mTargetChild = null;
