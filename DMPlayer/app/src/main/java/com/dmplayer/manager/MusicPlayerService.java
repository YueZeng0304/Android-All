...
public class MusicPlayerService extends Service implements AudioManager.OnAudioFocusChangeListener, NotificationManager.NotificationCenterDelegate {//服务
    ...
    @SuppressLint("NewApi")
    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {//开始命令
        try {
    ...
    @SuppressLint("NewApi")
    private void createNotification(SongDetail mSongDetail) {//创建通知
        try {
            ...
            RemoteViews simpleContentView = new RemoteViews(getApplicationContext().getPackageName(), R.layout.player_small_notification);//加载布局
            ...
            if (MediaController.getInstance().isAudioPaused()) {//是否暂停
            ...
            notification.contentView.setTextViewText(R.id.player_song_name, songName);//歌曲名
            notification.contentView.setTextViewText(R.id.player_author_name, authorName);//作者名
            if (supportBigNotifications) {
...
//                notification.bigContentView.setTextViewText(R.id.player_albumname, albumName);
    ...
    public void setListeners(RemoteViews view) {//各按钮监听器
        try {
    ...
    @Override
    public void onAudioFocusChange(int focusChange) {//播放焦点改变
        if (ignoreAudioFocus) {
    ...
    @Override
    public void didReceivedNotification(int id, Object... args) {//是否已经接受通知
