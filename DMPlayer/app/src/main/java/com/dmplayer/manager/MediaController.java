...
public class MediaController implements NotificationManager.NotificationCenterDelegate, SensorEventListener {
    ...
    public static MediaController getInstance() {//得到其他类数据
    ...
    public boolean isPlayingAudio(SongDetail messageObject) {//是否正在播放
    ...
    public boolean isAudioPaused() {//暂停
        return isPaused;
    }


    public void playNextSong() {//下一首
        playNextSong(false);
    }

    public void playPreviousSong() {//上一首
    ...
    private void stopProgressTimer() {//停止进度条
    ...
    private void stopProximitySensor() {//停止接近感应器
        if (ignoreProximity) {//忽略
            return;
        }
        try {
    ...
    public boolean playAudio(SongDetail mSongDetail) {//播放
        if (mSongDetail == null) {//不存在
            return false;
        }
    ...
    private void playNextSong(boolean byStop) {//下一首
        ArrayList<SongDetail> currentPlayList = shuffleMusic ? MusicPreferance.shuffledPlaylist : MusicPreferance.playlist;
    ...
    public boolean pauseAudio(SongDetail messageObject) {//暂停
        stopProximitySensor();
    ...
    public boolean resumeAudio(SongDetail messageObject) {//继续
        if (audioTrackPlayer == null && audioPlayer == null || messageObject == null || MusicPreferance.playingSongDetail == null || MusicPreferance.playingSongDetail != null
                && MusicPreferance.playingSongDetail.getId() != messageObject.getId()) {
            return false;
        }
    ...
    private void startProgressTimer() {//开始进度条计时
        synchronized (progressTimerSync) {
            if (progressTimer != null) {
                try {
                    progressTimer.cancel();
                                ...
                                if (MusicPreferance.playingSongDetail != null && (audioPlayer != null || audioTrackPlayer != null) && !isPaused) {
                                    try {
                                        ...
                                        if (audioPlayer != null) {
                                            ...
                                            if (progress <= lastProgress) {//上次进度条
                                                return;
                                            }
                                        } else {
    ...
    public boolean setPlaylist(ArrayList<SongDetail> allSongsList, SongDetail current, int type_, int id_) {//设置播放列表
        type = type_;
        id = id_;
     ...
     * @param songList
     */
    public static void shuffleList(ArrayList<SongDetail> songs) {//随机模式列表
        if (MusicPreferance.shuffledPlaylist.isEmpty()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                swap(songList, i, change);//交换歌曲
            }
            MusicPreferance.shuffledPlaylist = songList;
        }
    }

    private static void swap(ArrayList<SongDetail> songList, int i, int change) {//交换
     ...
     * @return
     */
    public boolean seekToProgress(SongDetail mSongDetail, float progress) {//查看进度
     ...
     * @param stopService
     */
    public void cleanupPlayer(Context context, boolean notify, boolean stopService) {//清空播放器
     ...
     */
    public synchronized void storeResendPlay(final Context context, final SongDetail mDetail) {//收集 再次发送
     ...
     */
    public synchronized void storeFavoritePlay(final Context context, final SongDetail mDetail, final int isFav) {//收集 喜爱歌曲
    ...
    public synchronized void checkIsFavorite(final Context context, final SongDetail mDetail, final View v) {//判断是否收藏
