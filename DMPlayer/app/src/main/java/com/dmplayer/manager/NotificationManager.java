...
public class NotificationManager {//通知管理
    ...
    private class DelayedPost {//延迟推送
    ...
    public static NotificationManager getInstance() {//得到例子
    ...
    public void postNotificationName(int id, Object... args) {//传递通知名
        boolean allowDuringAnimation = false;
        postNotificationNameInternal(id, allowDuringAnimation, args);
    }

    public void postNotificationNameInternal(int id, boolean allowDuringAnimation, Object... args) {//内部
        if (DEBUG_VERSION) {
        ...
        if (objects != null && !objects.isEmpty()) {
            for (int a = 0; a < objects.size(); a++) {
                ...
                ((NotificationCenterDelegate) obj).didReceivedNotification(id, args);//是否接受过通知
            }
        }
        broadcasting--;
        if (broadcasting == 0) {
            ...
            if (addAfterBroadcast.size() != 0) {//广播后增加
    ...
    public void addObserver(Object observer, int id) {//增加观察器
        if (DEBUG_VERSION) {
            if (Thread.currentThread() != ApplicationDMPlayer.applicationHandler.getLooper().getThread()) {
    ...
    public void notifyNewSongLoaded(int id, Object... args) {//通知新歌加载
        ArrayList<Object> objects = observers.get(id);
        if (objects != null && !objects.isEmpty()) {
            for (int a = 0; a < objects.size(); a++) {
                Object obj = objects.get(a);
