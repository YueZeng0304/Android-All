...
public class MusicPlayerReceiver extends BroadcastReceiver {//音乐播放接收器  广播接收器

	@Override
	public void onReceive(Context context, Intent intent) {
		if (intent.getAction().equals(Intent.ACTION_MEDIA_BUTTON)) {//ACTION_MEDIA_BUTTON
			if (intent.getExtras() == null) {
				return;
			}
			KeyEvent keyEvent = (KeyEvent) intent.getExtras().get(Intent.EXTRA_KEY_EVENT);//ACTION_MEDIA_BUTTON
			if (keyEvent == null) {
				return;
			}
			if (keyEvent.getAction() != KeyEvent.ACTION_DOWN)//ACTION_DOWN
				return;
			...
			case KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE:
				if (MediaController.getInstance().isAudioPaused()) {
					MediaController.getInstance().playAudio(MediaController.getInstance().getPlayingSongDetail());//播放
				} else {
