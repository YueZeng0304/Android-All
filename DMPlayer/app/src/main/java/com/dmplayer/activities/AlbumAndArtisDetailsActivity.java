...
public class AlbumAndArtisDetailsActivity extends ActionBarActivity implements View.OnClickListener, ObservableScrollViewCallbacks, Slider.OnValueChangedListener,
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_forward:
                if (MediaController.getInstance().getPlayingSongDetail() != null)
                    MediaController.getInstance().playNextSong();//下一首
                break;

            case R.id.btn_backward:
                if (MediaController.getInstance().getPlayingSongDetail() != null)
                    MediaController.getInstance().playPreviousSong();//上一首
                break;

            case R.id.btn_suffel://是否随机
            ...
            case R.id.btn_toggle://是否单曲循环
                v.setSelected(v.isSelected() ? false : true);
                MediaController.getInstance().repeatMode = v.isSelected() ? 1 : 0;//重复
                MusicPreferance.setRepeat(context, (v.isSelected() ? 1 : 0));
                DMPlayerUtility.changeColorSet(context, (ImageView) v, v.isSelected());
                break;

            case R.id.bottombar_img_Favorite://收藏
                if (MediaController.getInstance().getPlayingSongDetail() != null) {
    ...
    @Override
    public void onScrollChanged(int scrollY, boolean firstScroll, boolean dragging) {//滚动
        int baseColor = color;
    ...
    private void getBundleValuse() {
        ...
        tv_albumname.setText(albumname);//include_albumdetailsongslist专辑名
        tv_title_fst.setText(title_one);//第一名字
        tv_title_sec.setText(title_sec);//第二名字
    }


    private void loadAlbumSongs(long id) {//加载专辑歌曲
    ...
    private void loadArtisSongs(long id) {//加载艺术家歌曲
        PhoneMediaControl mPhoneMediaControl = PhoneMediaControl.getInstance();
        PhoneMediaControl.setPhonemediacontrolinterface(new PhoneMediaControl.PhoneMediaControlINterface() {
    ...
    public class AllSongsListAdapter extends BaseAdapter {//歌曲配适器
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            convertView.setOnClickListener(new View.OnClickListener() {
                ...
                @Override
                public void onClick(View v) {//MediaController
            ...
            mViewHolder.imagemore.setOnClickListener(new View.OnClickListener() {
                ...
                @Override
                public void onClick(View v) {//菜单列表
                    try {
                        ...
                        popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                            ...
                            public boolean onMenuItemClick(MenuItem item) {
                                ...
                                switch (item.getItemId()) {
                                    case R.id.playnext://下一首
                                        break;
                                    case R.id.addtoque://加到列表
                                        break;
                                    case R.id.addtoplaylist://加入播放列表
                                        break;
                                    case R.id.gotoartis://跳转到艺术家
                                        break;
                                    case R.id.gotoalbum://跳转到专辑
                                        break;
                                    case R.id.delete://删除
                                        break;
                                    default:
    ...
    private void initiSlidingUpPanel() {
        mLayout = (SlidingUpPanelLayout) findViewById(R.id.sliding_layout);//slidinguppanelhelper.SlidingUpPanelLayou
        // songAlbumbg = (ImageView) findViewById(R.id.image_songAlbumbg);
        songAlbumbg = (ImageView) findViewById(R.id.image_songAlbumbg_mid);//include_slidingpanel_childtwo
        img_bottom_slideone = (ImageView) findViewById(R.id.img_bottom_slideone);//include_slidepanelchildtwo_topviewone
        ...
        imgbtn_toggle.setSelected((MusicPreferance.getRepeat(context) == 1) ? true : false);//切换模式
        ...
        imgbtn_suffel.setSelected(MusicPreferance.getShuffel(context) ? true : false);//重复
        ...
        mLayout.setPanelSlideListener(new SlidingUpPanelLayout.PanelSlideListener() {//进度条
            @Override
    ...
    private void loadAlreadyPaing() {//加载已在收藏列表的歌曲
    ...
    private static class AnimateFirstDisplayListener extends SimpleImageLoadingListener {//驱动第一手歌
    ...
    public void loadSongsDetails(SongDetail mDetail) {//加载歌曲细节
        String contentURI = "content://media/external/audio/media/" + mDetail.getId() + "/albumart";
        ...
        if (txt_timetotal != null) {//include_slidepanelchildtwo_bottomview进度条
    ...
    @Override
    public void newSongLoaded(Object... args) {//加载新歌曲
        MediaController.getInstance().checkIsFavorite(context, (SongDetail) args[0], img_Favorite);
    }


    private void updateTitle(boolean shutdown) {//更新标题
        SongDetail mSongDetail = MediaController.getInstance().getPlayingSongDetail();
    ...
    private void updateProgress(SongDetail mSongDetail) {//更新进度
    ...
    private void PlayPauseEvent(View v) {//停止
        if (MediaController.getInstance().isAudioPaused()) {
