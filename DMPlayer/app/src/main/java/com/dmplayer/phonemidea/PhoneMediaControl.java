...
public class PhoneMediaControl {//手机媒体控制器

    private Context context;
    private Cursor cursor = null;
    private static volatile PhoneMediaControl Instance = null;

    public static enum SonLoadFor {//各属性
    ...
    public void loadMusicList(final Context context, final long id, final SonLoadFor sonloadfor, final String path) {//加载音乐列表
        ...
        AsyncTask<Void, Void, Void> task = new AsyncTask<Void, Void, Void>() {
            ...
            @Override
            protected void onPostExecute(Void aVoid) {//传递执行指令
    ...
    public ArrayList<SongDetail> getList(final Context context, final long id, final SonLoadFor sonloadfor, final String path) {//得到列表
...
//                sortOrder = MediaStore.Audio.Media.DATE_ADDED + " desc";
    ...
    private ArrayList<SongDetail> getSongsFromCursor(Cursor cursor) {//通过光标得到歌曲
    ...
    private ArrayList<SongDetail> getSongsFromSQLDBCursor(Cursor cursor) {//通过数据库光标得到歌曲
        ArrayList<SongDetail> generassongsList = new ArrayList<SongDetail>();
        try {
            if (cursor != null && cursor.getCount() >= 1) {
    ...
    public static void runOnUIThread(Runnable runnable, long delay) {//在UI线程上运行
        if (delay == 0) {
    ...
    public static void setPhonemediacontrolinterface(PhoneMediaControlINterface phonemediacontrolinterface) {//设置手机媒体控制界面
