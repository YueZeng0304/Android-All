...
public class Utils {//工具
    ...
    public static HashMap<String, String> getItem(int position){//得到项目
        return arraylist.get(position);
    }

    public static void sort(){//arraylist排序
    ...
    public static void MillisToDate(ArrayList<HashMap<String,String>> arraylist){//tempTimeMillis时间存为日期
        int size = arraylist.size();
    ...
    public static void DateToMillis(ArrayList<HashMap<String,String>> arraylist){//日期存为tempTimeMillis时间
        int size = arraylist.size();
        for(int i=0;i<size;i++){
    ...
    public static String format(int hourOfDay) {//格式
    ...
    public static String toDateString(Calendar calendar){//日期字符串
        String day = null;
    ...
    public static String timeTransfer(int i){//日期转变 昨天
        Time time = new Time();
        long tempLong = Long.parseLong(arraylist.get(i).get("datetime"));
        tempTimeMillis[i] = tempLong;
        //日期
        time.setToNow();//现在时间
        String dateNow = time.toString().substring(0,8);//日期
        String timeNow = time.toString().substring(9,13);//时间
        time.set(tempLong);
        String datePast = time.toString().substring(0,8);
        String timePast = time.toString().substring(9,13);
        long tempDate = Long.parseLong(dateNow)-Long.parseLong(datePast);//现在与过去时间差
    ...
    public static String timeTransfer(String i){//日期转变 今天 明天 后天
        Time time = new Time();
        time.setToNow();
