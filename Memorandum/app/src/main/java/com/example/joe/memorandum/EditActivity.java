...
public class EditActivity extends Activity{//编辑页面
...
//注册各组件
        ...
        user = new UserInfo();//一条信息
        user.setAlerttime(alerttime);
        timeSetButton.setOnClickListener( new OnClickListener(){//设置时间
            @Override
            public void onClick(View v) {
                timeSetDialog = new TimeSetDialog(EditActivity.this);//设置时间对话框
                //添加监听器，当dialog消失即执行cancel()方法时触发的事件
                timeSetDialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
                    @Override
                    public void onCancel(DialogInterface dialog) {
                        alerttime = timeSetDialog.alerttime;//设置时间
                        if(alerttime != null)
                            alertTextView.setText(Utils.timeTransfer(alerttime));//文本显示设置时间
                        else
                            alertTextView.setText("");
                        calendar = timeSetDialog.calendar;//设置日期
                        user.setAlerttime(alerttime);
                    }
                });
                timeSetDialog.show();
        ...
        backButton.setOnClickListener(new OnClickListener(){//返回
            @Override
            public void onClick(View v) {
                onBackPressed();
            }
...
//		String minute = time.minute<10?"0"+time.minute:String.valueOf(time.minute);
        tempDatetime1 = month+"月"+day+"日"+'\n'+Utils.format(hour)+":"+Utils.format(minute);//月 日 时 分
...
//	}
    ...
    @Override
    public void onBackPressed() {//退出
        super.onBackPressed();
        edittext = (EditText)findViewById(R.id.editText);
        Time time = new Time();
        time.setToNow();//设置时间为现在

        user.setAlerttime(alerttime);
        datetime =""+time.toMillis(true);
        user.setDatetime(datetime);
        time.set(time.toMillis(true));//设置时间

        content = edittext.getText().toString();//内容为文本框输入字符
        ...
        if((!content.isEmpty() && !tempContent.equals(content)) || !alerttime.equals("") && !alerttime.equals(tempAlerttime)){                   //如果内容非空且已经被更改，则更新显示和数据库
            ...
            HashMap<String,String> map = new HashMap<String,String>();//map保存数据
    ...
    private void alertSet(){//设置闹钟
        Intent intent = new Intent("android.intent.action.ALARMRECEIVER");
        intent.putExtra("datetime", datetime);
        intent.putExtra("content", content);
        intent.putExtra("alerttime",alerttime);
        PendingIntent pendingIntent = PendingIntent.getBroadcast(EditActivity.this, 0, intent, 0);//广播
        AlarmManager alarmManager = (AlarmManager)getSystemService(ALARM_SERVICE);//闹钟服务
